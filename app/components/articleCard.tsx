...
const ArticleCard: React.FC<{card: Article, flex: boolean}> = ({card, flex}) => {
  ...
  key={card.article_url} className={`${flex ? "w-72 sm:w-80":"max-w-lg"} overflow-hidden hover:scale-[102%] transition-transform bg-white rounded-lg shadow-lg flex flex-col gap-2`}>
    ...
    src={`${process.env.STRAPI_API_URL}${card?.image_banner?.data?.attributes?.formats.small ? card?.image_banner?.data?.attributes?.formats.small.url : card?.image_banner?.data?.attributes?.url}`}/>
    <h3 className="px-6">
      {card?.title}
    </h3>
    <p className="px-6 italic text-sm font-bold">{card?.article_filters?.data?.map(a => {
      return<>
      {a.attributes?.tag}
      </>
    })}</p>
